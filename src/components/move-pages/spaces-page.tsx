...
export function SpacesWorkspacePage() {
  ...
  const countsByKind = useMemo(() => {
    ...
    for (const space of spaces ?? []) {
      map.set(space.kind, (map.get(space.kind) ?? 0) + 1);
    }
    return kindOrder
      .map((entry) => ({
        kind: entry,
        label: kindLabels[entry],
            ...
            <CardHeader>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Rooms and zones first; setup actions stay separate so the
                    records are visible immediately.
                  </CardDescription>
                </div>
                {spaces ? (
            ...
            <CardContent className="space-y-4">
              {countsByKind.length ? (
                <div className="flex gap-2 overflow-x-auto pb-1 md:flex-wrap md:overflow-visible md:pb-0">
                  {countsByKind.map((entry) => (
                    <Badge key={entry.kind} variant="outline">
              ...
              {spaces === undefined ? (
                ...
                <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
                  No spaces yet. Add origin and destination rooms first;
                  transport resources/zones can be linked as the workflow
                  matures.
                </div>
              ) : (
                grouped.map((group) => (
                      ...
                      {group.spaces.map((space) => (
                        ...
                        >
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            <Badge
                              variant={space.photoCount ? "secondary" : "outline"}
                            >
                              <Camera
        ...
        <TabsContent value="add">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Spaces give AI and Layout Studio durable targets instead of
                loose room text.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form
                className="grid gap-3 lg:grid-cols-[180px_1fr_160px_1.2fr_auto]"
              ...
              >
                <select
                  ...
                  onChange={(event) =>
                    setKind(event.target.value as SpaceKind)
                  }
                >
                  {kindOrder.map((entry) => (
