...
type ResourceReport = EstimateReport["resourceReports"][number];
type PlannerFilter =
  | "all"
  | "unassigned"
  | "warnings"
        ...
        >
          ...
          <TabsContent value="units">
            <MovableUnitsPanel
              ...
              resourceReportById={reportByResourceId}
...
function MovableUnitsPanel({
  ...
  loading: boolean;
  moveId: Id<"moves"> | null;
  resourceReportById: Map<Id<"transportResources">, ResourceReport>;
  ...
  return (
    <div className="space-y-3 rounded-md border border-border p-3">
      ...
      <LoadPlanRollups
        resourceReportById={resourceReportById}
        resourcesWithZones={resourcesWithZones}
        units={units}
      />
...
function createLoadPlanRollupGroup({
  key,
  label,
  subtitle,
  priority = 0,
...
function addUnitToLoadPlanRollup(
  group: LoadPlanRollupGroup,
  unit: MovableUnit,
) {
  group.unitCount += 1;
  if (unit.kind === "box") {
    ...
    group.looseItemCount += 1;
  }
  if (
    typeof unit.estimatedWeightLb === "number" &&
    Number.isFinite(unit.estimatedWeightLb)
  ) {
    group.knownWeightLb += unit.estimatedWeightLb;
  }
  if (
    typeof unit.estimatedVolumeCuFt === "number" &&
    Number.isFinite(unit.estimatedVolumeCuFt)
  ) {
    group.knownVolumeCuFt += unit.estimatedVolumeCuFt;
  }
  if (unit.assignmentState === "assigned") {
    group.assignedCount += 1;
  } else if (unit.assignmentState === "owner") {
    group.personalTransportCount += 1;
  } else {
    group.unassignedCount += 1;
  }
  if (unit.assignmentLocked) {
    group.lockedCount += 1;
  }
  group.missingMeasurementCount += unit.missingFields.length;
  if (group.examples.length < 3) {
    group.examples.push(unit.name);
  }
}

function buildTransportLoadPlanRollups({
  resourceReportById,
  resourcesWithZones,
  units,
}: {
  ...
  const groups = new Map<string, LoadPlanRollupGroup>();

  for (const [index, { resource, zones }] of resourcesWithZones.entries()) {
    groups.set(
      `resource:${resource._id}`,
      createLoadPlanRollupGroup({
        key: `resource:${resource._id}`,
        label: resource.name,
        subtitle: zones.length
          ? `${resource.type} with ${zones.length} ${zones.length === 1 ? "zone" : "zones"}`
          : resource.type,
        priority: index + 10,
        resourceReport: resourceReportById.get(resource._id),
      }),
    );
  }
  ...
  for (const unit of units) {
    ...
    if (unit.assignedResourceId) {
      groupKey = `resource:${unit.assignedResourceId}`;
      group =
        groups.get(groupKey) ??
        createLoadPlanRollupGroup({
          key: groupKey,
          label: unit.assignmentLabel || "Assigned load",
          subtitle: "Assigned outside the current resource list",
          priority: 500,
        });
    } else if (unit.assignmentState === "owner") {
      groupKey = "personal";
      group =
        ...
        createLoadPlanRollupGroup({
          ...
          subtitle: "Rides with the owner or another private vehicle",
          priority: 900,
        });
    } else if (!group) {
      group = createLoadPlanRollupGroup({
        key: groupKey,
        label: "Unassigned",
        subtitle: "Needs a truck, trailer, storage, or owner decision",
        priority: 0,
      });
    }

    addUnitToLoadPlanRollup(group, unit);
...
function buildRoomLoadPlanRollups(
  units: readonly MovableUnit[],
): LoadPlanRollupGroup[] {
  const groups = new Map<string, LoadPlanRollupGroup>();
  ...
  for (const unit of units) {
    ...
    const groupKey = `room:${roomLabel}`;
    const group =
      groups.get(groupKey) ??
      createLoadPlanRollupGroup({
        key: groupKey,
        label: roomLabel,
        subtitle: "Source room or staging area",
      });

    addUnitToLoadPlanRollup(group, unit);
    group.priority =
      group.unassignedCount * 50 +
      group.missingMeasurementCount * 10 +
      group.unitCount;
    groups.set(groupKey, group);
  }
...
function LoadPlanRollups({
  ...
  const transportRollups = buildTransportLoadPlanRollups({
    resourceReportById,
    resourcesWithZones,
    units,
  });
  const roomRollups = buildRoomLoadPlanRollups(units);

  return (
    <section
      aria-label="Load plan rollups"
      className="rounded-md border border-border bg-background/65 p-3"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-medium">Load plan rollups</p>
          <p className="mt-1 text-xs leading-5 text-muted-foreground">
            Read the rough load by transport and by room before changing
            assignments.
          </p>
        </div>
        <Badge variant="outline">
          {units.length} movable {unitWord(units.length)}
        </Badge>
      </div>

      <div className="mt-3 grid gap-3 xl:grid-cols-2">
        <LoadPlanRollupColumn
          ariaPrefix="Transport"
          emptyText="No transport resources or assignments yet."
          rollups={transportRollups}
          title="By transport"
          showCapacity
        />
        <LoadPlanRollupColumn
          ariaPrefix="Room"
          emptyText="No rooms or staging areas yet."
          rollups={roomRollups}
          title="By room"
        />
      </div>
...
function LoadPlanRollupColumn({
  ...
  ariaPrefix: string;
  emptyText: string;
  rollups: readonly LoadPlanRollupGroup[];
  ...
  return (
    <div className="rounded-md border border-border bg-muted/10 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge variant="secondary">{rollups.length}</Badge>
      </div>
      {rollups.length ? (
...
function LoadPlanRollupCard({
  ariaPrefix,
  rollup,
  showCapacity,
}: {
  ariaPrefix: string;
  rollup: LoadPlanRollupGroup;
  showCapacity: boolean;
}) {
  const weightValue =
    showCapacity && rollup.resourceReport
      ? rollup.resourceReport.estimatedWeightLb
      : rollup.knownWeightLb;
  const volumeValue =
    showCapacity && rollup.resourceReport
      ? rollup.resourceReport.estimatedVolumeCuFt
      : rollup.knownVolumeCuFt;
  const unitCount =
    showCapacity && rollup.resourceReport?.assignedUnitCount
      ? Math.max(rollup.unitCount, rollup.resourceReport.assignedUnitCount)
      : rollup.unitCount;

  return (
    <article
      aria-label={`${ariaPrefix} rollup for ${rollup.label}`}
      className="rounded-md border border-border bg-background/70 p-3"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-sm font-medium">{rollup.label}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {rollup.subtitle}
          </p>
        </div>
        <Badge variant={unitCount ? "outline" : "secondary"}>
          {unitCount} {unitWord(unitCount)}
        </Badge>
      </div>

      <div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
        <MetricPill label="Boxes" value={rollup.boxCount} />
        <MetricPill label="Loose" value={rollup.looseItemCount} />
        <MetricPill label="Weight" value={formatKnownWeight(weightValue)} />
        <MetricPill label="Volume" value={formatKnownVolume(volumeValue)} />
      </div>
      ...
      {showCapacity && rollup.resourceReport ? (
        <div className="mt-3 grid gap-2 text-xs">
          <CapacityLine
            ...
            value={rollup.resourceReport.estimatedWeightLb}
            max={rollup.resourceReport.maxWeightLb}
            percent={rollup.resourceReport.weightPercent}
            unit="lb"
          />
          <CapacityLine
            label="Volume capacity"
            value={rollup.resourceReport.estimatedVolumeCuFt}
            max={rollup.resourceReport.maxVolumeCuFt}
            percent={rollup.resourceReport.volumePercent}
            unit="cu ft"
          />
        </div>
      ) : null}
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {rollup.unassignedCount ? (
          <Badge variant="secondary">Need load {rollup.unassignedCount}</Badge>
        ) : null}
        {rollup.personalTransportCount ? (
          <Badge variant="outline">
            Personal {rollup.personalTransportCount}
          </Badge>
        ) : null}
        {rollup.assignedCount ? (
          <Badge variant="outline">Assigned {rollup.assignedCount}</Badge>
        ) : null}
        {rollup.lockedCount ? (
          <Badge variant="outline">Locked {rollup.lockedCount}</Badge>
        ) : null}
        {rollup.missingMeasurementCount ? (
          <Badge variant="outline">Gaps {rollup.missingMeasurementCount}</Badge>
        ) : null}
      </div>

      {rollup.examples.length ? (
        <p className="mt-3 text-xs leading-5 text-muted-foreground">
          {rollup.examples.join(", ")}
        </p>
      ) : (
        <p className="mt-3 text-xs leading-5 text-muted-foreground">
          No units assigned here yet.
        </p>
      )}
    </article>
  );
