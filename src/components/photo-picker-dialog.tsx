...
type PhotoPickerDialogBodyProps = {
...
function PhotoPickerDialogBody({
  householdId,
  moveId,
  target,
  targetLabel,
  ...
  async function attach(photoId: Id<"itemPhotos">) {
    ...
    try {
      await updateEvidence({
        ...
        photoId,
        ...(target.kind === "item"
          ? { itemId: target.itemId, boxId: null }
          : { boxId: target.boxId, itemId: null }),
      });
      onAttached?.();
      setAttachingId(null);
      onOpenChange(false);
