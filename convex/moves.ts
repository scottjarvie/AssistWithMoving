...
//
// The two lists below are verified against the schema by
...
const MOVE_INDEXED_PURGE_TABLES: Array<{ table: string; index: string }> = [
  ...
  { table: "floorplanEvidenceRecords", index: "by_move_status" },
  { table: "floorplanObservations", index: "by_move_status" },
  { table: "floorplanRelationships", index: "by_move_status" },
  { table: "floorplanMeasurements", index: "by_move_status" },
  { table: "floorplanCalculationRecords", index: "by_move_status" },
  { table: "floorplanSolveRuns", index: "by_move" },
...
// moveRoleGrants is keyed by (householdId, moveId) — handled on its own.
...
async function purgeMoveData(
  ...
  const grants = await ctx.db
    ...
    .withIndex("by_household_move", (q: any) =>
      q.eq("householdId", householdId).eq("moveId", moveId),
    )
    .collect();
  await deleteDocs("moveRoleGrants", grants);
