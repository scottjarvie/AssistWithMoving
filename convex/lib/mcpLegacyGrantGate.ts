 ...
 */
import { movingScopes, type MovingScope } from "./aiGrants";
 ...
 *
 * Pure on purpose. Everything it needs — the scopes the person's current grant
 * carries, or the reason there is no usable grant — is resolved once per
 * request by the caller and handed in.
 */
export function decideLegacyGatewayAccess(input: {
  ...
  const { toolName, grantedScopes, block } = input;
  ...
  const scope = legacyScopeForTool(toolName);
  if (!scope) {
    return {
      allowed: false,
  ...
  if (block === "noIdentity") {
    return {
      ...
      reason:
        "Sign in with your Assist With Moving account to use these tools.",
    };
  }
  if (block === "noProfile") {
    return {
  ...
  if (!grantedScopes.includes(scope)) {
    return {
      ...
      scope,
      refusalCode: "GRANT_SCOPE_MISSING",
      reason: `This connection was not approved for ${scope}. The person can add it at ${GRANT_MANAGER_URL}, then you reconnect.`,
    };
  }
