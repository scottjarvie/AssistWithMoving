...
describe("MCP OAuth helpers", () => {
  ...
  it("uses forwarded host headers when the runtime request URL is internal", () => {
    process.env.NEXT_PUBLIC_APP_URL = "http://localhost:3827";
    const request = new Request("http://127.0.0.1:3000/api/mcp", {
      headers: {
        "x-forwarded-host": "movingmanifest-preview.vercel.app",
        "x-forwarded-proto": "https",
      },
    });

    expect(mcpProtectedResourceMetadata(request).resource).toBe(
      "https://movingmanifest-preview.vercel.app/api/mcp"
    );
  });
});
