...
describe("LoadPlannerBoard task tabs", () => {
  ...
  it("opens on movable units and keeps board, bulk assignment, and loose-item work separate", async () => {
    ...
    const rollups = screen.getByLabelText("Load plan rollups");
    expect(within(rollups).getByText("By transport")).toBeInTheDocument();
    expect(within(rollups).getByText("By room")).toBeInTheDocument();
    const truckRollup = within(rollups).getByLabelText(
      "Transport rollup for Moving truck",
    );
    expect(truckRollup).toHaveTextContent("Moving truck");
    expect(truckRollup).toHaveTextContent("Weight capacity");
    expect(truckRollup).toHaveTextContent("0 lb");
    const unassignedRollup = within(rollups).getByLabelText(
      "Transport rollup for Unassigned",
    );
    expect(unassignedRollup).toHaveTextContent("Kitchen essentials");
    expect(unassignedRollup).toHaveTextContent("Need load 1");
    expect(unassignedRollup).toHaveTextContent("18 lb");
    const personalRollup = within(rollups).getByLabelText(
      "Transport rollup for Personal transport",
    );
    expect(personalRollup).toHaveTextContent("Floor lamp");
    expect(personalRollup).toHaveTextContent("Personal 1");
    const kitchenRollup = within(rollups).getByLabelText(
      "Room rollup for Kitchen",
    );
    expect(kitchenRollup).toHaveTextContent("Kitchen essentials");
    expect(kitchenRollup).toHaveTextContent("Need load 1");
    expect(kitchenRollup).toHaveTextContent("Gaps 2");
    expect(kitchenRollup).toHaveTextContent("18 lb");
    const livingRoomRollup = within(rollups).getByLabelText(
      "Room rollup for Living room",
    );
    expect(livingRoomRollup).toHaveTextContent("Floor lamp");
    expect(livingRoomRollup).toHaveTextContent("Personal 1");
    expect(livingRoomRollup).toHaveTextContent("Gaps 3");
