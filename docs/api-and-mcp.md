...
| `update_move` | Update an existing move's basics — name, status (`planning`, `active`, or `completed` only), origin/destination, dates, driving distance (miles), travel time (minutes), notes, documentation profiles, and the official weight allowance. `distanceMiles`, `travelMinutes`, `notes`, and `moveLevelWeightAllowanceLb` accept `null` to clear. Wraps `PATCH /api/v1/moves/{moveId}`. |
