...
| DCR-default with no CIMD path | `convex/lib/mcpClientIdentity.ts` prefers a metadata document, requires it to name its own URL, validates redirects, binds its digest to the grant, and refuses rather than downgrades on failure | Nothing — resolved 2026-08-17: Clerk production DCR is live fleet-wide and is the approved soft-launch path; the metadata-document path is deferred by decision and stays implemented as an honest preference |
...
| Unproved private media delivery | Bytes are fetched server-side through the web app's own display path (Cloudflare Images or a B2 signed URL) and returned as inline blocks scoped to what the photo is attached to, with no storage link — now budgeted per image and per batch, stepping down to a smaller variant before dropping a photo, and naming every omission with an actionable reason (`MOV-0038`) | The storage host itself, inside a real AI product |
