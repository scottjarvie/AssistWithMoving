...
> `MOV-WO-008`; re-audited 2026-08-16 against `origin/main` `8a064303`, all
...
> `origin/main` at `8a064303` plus the explicitly named live anonymous checks.
...
| API and MCP source surfaces | Documented REST API; a canonical stateless Streamable HTTP OAuth resource in `convex/httpRoutes/mcp.ts` + `convex/mcpPlanning.ts`; a persisted compatibility gateway in `convex/mcp*.ts`; and a separate API-key HTTP/stdio registry in `src/app/api/mcp` + `mcp-server/` | **Current/verified production foundation** for anonymous discovery/challenge, door separation, official-SDK consent/token exchange, exact move isolation, complete source-backed save, replay, correction, normal UI reflection, refresh/client/session cleanup and fixture purge. The complete connection is **Partial** because product scopes/grants, immediate OAuth revocation, metadata-document-first client identity, named-product media/reconnect, and canonical Queue transitions are not proved. `MOV-WO-010` owns the Proposed alignment program |
| Granular chosen-AI grants and external-action handoffs | Scoped API keys, OAuth/client identity, Queue claims and selected audit fields exist. No complete product grant binds one OAuth client to selected moves/data, separate operations, approval, expiry, last use and immediate revocation. No external-action contract binds selected data/intent to an attributable result/failure/return | **Partial product-grant foundations; Later complete contracts.** `MOV-0023` owns product grants inside Moving; `MOV-0036` preserves external-action handoffs outside `MOV-WO-010`. Do not claim every AI can connect or receive an outside-action handoff |
