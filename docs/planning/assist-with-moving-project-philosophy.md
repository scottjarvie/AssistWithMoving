...
| API and MCP source surfaces | Documented REST API; a canonical stateless OAuth resource in `convex/httpRoutes/mcp.ts` + `convex/mcpPlanning.ts`; a persisted compatibility gateway in `convex/mcp*.ts`; and a separate API-key HTTP/stdio registry in `src/app/api/mcp` + `mcp-server/` | **Current/verified production foundation** at merge `0a5e0eb` for deployment, anonymous discovery/challenge, door separation, docs and signed-out privacy; isolated synthetic protocol and complete-result proof covers the authenticated data path without production data. A sanctioned production client proved consent/token exchange and the Clerk no-`aud` shape; the compatible verifier and complete named-client tool/UI/revocation/cleanup receipt remain **active / Partial** in `MOV-0029`. Private media and disconnect/reconnect beyond the run cleanup remain **Unknown** |
