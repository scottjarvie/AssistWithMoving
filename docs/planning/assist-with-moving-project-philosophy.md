...
> receipts described in the evidence record below
>
...
| Public truth and Coming soon | Source includes home, separate FAQ, `/ai`, `/ai.txt`, `llms.txt`, `llms-full.txt`, and `/updates`. The home page does not contain the FAQ. | `/ai`, `/mcp/guide`, `/ai.txt` and the MCP discovery/refusal boundaries are **Current/verified in production** at merge `0a5e0eb`; other public claims still require their own environment/user-path proof. **Coming soon** is reserved for approved, scheduled soft-launch work; none is assigned here. |
...
| API and MCP source surfaces | Documented REST API; a canonical stateless OAuth resource in `convex/httpRoutes/mcp.ts` + `convex/mcpPlanning.ts`; a persisted compatibility gateway in `convex/mcp*.ts`; and a separate API-key HTTP/stdio registry in `src/app/api/mcp` + `mcp-server/` | **Current/verified production foundation** at merge `0a5e0eb` for deployment, anonymous discovery/challenge, door separation, docs and signed-out privacy; isolated synthetic protocol, OAuth-audience and complete-result proof covers the authenticated data path without production data. Named-client consent/refresh, private media and disconnect/reconnect remain **Partial / Unknown** |
