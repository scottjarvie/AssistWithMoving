...
>
> **Capability evidence last verified:** 2026-08-12 against the protected-release
> candidate in `MOV-WO-005`, its isolated stateless MCP protocol/auth/data proof,
> and the earlier provider receipts described in the evidence record below
>
...
|---|---|---|
| Person / Assist workspace / your AI | The product philosophy, `/ai` copy, scoped keys, canonical stateless OAuth MCP and compatibility/API-key surfaces separate the person's authority, Moving's durable record and the person's chosen AI. Older public copy still often says “AI assistant” and leads with inventory rather than the full three-way promise. | **Current/verified repository foundation; Later alignment gap.** Adopt “Assist your AI, so it can assist you with moving” and the house sentence “Assist you and your AI with every part of your move.” |
...
| MCP and `/ai` | Source routes provide a canonical stateless OAuth `/mcp` with eight move-workflow tools, a persisted compatibility catalog at `/mcp/connect`, and separate API-key `/api/mcp` plus stdio. Public `/ai`, `/ai/start`, `/ai.txt`, `llms.txt` and `llms-full.txt` state the door and Queue boundaries; AI settings live at `/settings/ai-connections` and within `/settings`. | **Current/verified repository source with isolated synthetic protocol/auth/data proof; production and named-client proof remain Unknown until exact release receipts exist.** Canonical `/settings/ai` is a **Later alignment gap**. Public pages must list only the catalog attached to their named door. |
...
| Public truth and Coming soon | Source includes home, separate FAQ, `/ai`, `/ai.txt`, `llms.txt`, `llms-full.txt`, and `/updates`. The home page does not contain the FAQ, and source presence does not prove live claims. | **Current/verified source foundation; Unknown deployment until exact release proof.** Public claims require an environment/user-path proof. **Coming soon** is reserved for approved, scheduled soft-launch work; none is assigned here. |
...
| API and MCP source surfaces | Documented REST API; a canonical stateless OAuth resource in `convex/httpRoutes/mcp.ts` + `convex/mcpPlanning.ts`; a persisted compatibility gateway in `convex/mcp*.ts`; and a separate API-key HTTP/stdio registry in `src/app/api/mcp` + `mcp-server/` | **Current/verified repository source with isolated synthetic protocol, OAuth-audience and complete-result proof; deployment and named-client paths remain Unknown until exact receipts exist.** Reverify auth consent/refresh, private media, disconnect/reconnect and production behavior before a named-client availability claim |
