...
> the exact fail-closed state-versus-software publication boundary; and the
...
- **Partial** means a useful implemented or deployed foundation has a named
  permission, lifecycle, environment, or proof gap and cannot support the
  complete claim.
...
| **Partial** | A useful foundation is implemented or deployed, but a named permission, lifecycle, environment or proof element is missing | State the working part and the exact gap; never present the complete capability as Current |
...
> **Time-sensitive evidence — reverified 2026-08-16:** This table describes
> `origin/main` at `8a064303`, reverified against `1d0b76d` on the same date,
> plus the explicitly named live anonymous checks.
> It is not proof that the same
...
| Questions, Move Brief and queue | Derived readiness questions; a bounded MCP move brief; compact move/context summaries; and a per-user four-state Queue with directives, evidence, claim expiry, delegation, AI question, summary, activity and result references. The canonical OAuth surface can now list only actionable granted work, claim and release it under a lease, ask the smallest **Needs you** question, and complete it — including a one-call save that finishes the handoff in the same approval | **Current/verified repository foundation.** The complete cross-domain Move Brief and attachable question system are **Later**. Queue transitions are held behind a distinct `moving.queue.work` grant, so a work-write grant links a result and leaves the state to the person |
...
| API and MCP source surfaces | Documented REST API; a canonical stateless Streamable HTTP OAuth resource in `convex/httpRoutes/mcp.ts` + `convex/mcpPlanning.ts` with fifteen grant-gated workflow tools; a persisted compatibility gateway in `convex/mcp*.ts`; and a separate API-key HTTP/stdio registry in `src/app/api/mcp` + `mcp-server/` | **Current/verified repository foundation** for anonymous discovery/challenge, door separation, exact move isolation, grant-filtered tool discovery, complete source-backed save, replay, correction, canonical Queue claim/question/completion, reversible archive, normal UI reflection and fixture purge. The complete connection is **Partial**: `MOV-WO-010` closed product scopes/grants, immediate revocation, metadata-document-first client identity and Queue transitions in source, but no deployment, no provider metadata-document enablement, and no named AI product lifecycle is proved |
| Granular chosen-AI grants and external-action handoffs | An `aiGrants` record binds one OAuth client to selected moves, five separate product scopes, an approval, a frozen consent snapshot, an expiry, last use and revocation, re-read from the database on every discovery and every tool call. `aiGrantActivities` keeps an owner-readable use-and-refusal trail. Tested for scope separation, move separation, cross-client isolation, expiry and immediate revocation. No external-action contract binds selected data/intent to an attributable result/failure/return | **Current/verified repository contract for Moving-side grants; Later for deployment proof and for external-action handoffs.** Revocation is immediate in source and untested in production. `MOV-0036` preserves external-action handoffs outside `MOV-WO-010`. Do not claim any particular AI can connect |
...
| Bring Your AI alignment | [Moving MCP/OAuth alignment plan](moving-bring-your-ai-mcp-oauth-alignment.md), [Work Order `MOV-WO-010`](../tracker/work-orders/MOV-WO-010.md), and [family-standard draft receipt](https://github.com/scottjarvie/assistwithlife/pull/16) | Moving-specific Current/Partial/Later audit, product grants, Queue/evidence/save workflow, metadata-first client identity, human connection experience, and proof plan; Proposed work is not Current capability |
...
1. classify every material claim with one of this document's six alignment
   statuses;
...
connection, prove Client ID Metadata Documents first or record why DCR fallback
was required; product read/evidence/write/Queue/archive scopes and consent;
credential handling; person-visible activity and immediate revocation; failure,
reconnect and stale-tool behavior; cleanup; and the exact user path. An official
SDK harness or token exchange does not prove Claude, ChatGPT, Codex, Grok,
Hermes, or another named AI product. Before naming an external action,
...
- **1.9.0 · 2026-08-16** — recorded the implemented `MOV-WO-010` system: five
  ...
  snapshot and expiry, per-call grant re-reads that make revocation immediate,
  grant-filtered tool discovery, Client ID Metadata Document-first client
  identity with a labelled dynamic-registration fallback, canonical Queue
  claim/question/completion with a one-call finish, reversible archive as the
  only destructive verb, `/settings/ai` as the grant manager, and a scripted
  nine-step lifecycle harness. This is repository and test evidence only. The
  connection stays **Partial**: no deployment, no provider metadata-document
  enablement, and no named AI product lifecycle is proved, so every client
  remains **Unknown** and the four doors remain distinct.
- **1.8.0 · 2026-08-16** — adopted the Bring Your AI implementation standard
  as a separate delivery reference; re-audited fresh `origin/main`, all four
  agent doors and live anonymous OAuth discovery; added Partial as a first-class
  truth state; preserved the Current stateless eight-tool and SDK foundations;
  and recorded product scopes/grants, immediate revocation, Client ID Metadata
  Documents, canonical Queue work, `/settings/ai`, and real named-client proof
  as the missing system proposed in `MOV-WO-010`.
